...
This is an important demo because it proves that the engine is not simply an in-memory key-value store.
...
The non-zero WAL file shows that the operation generated WAL data.
...
> "The B+ Tree itself is in memory, so after a process restart the tree has to be reconstructed. I achieve that by writing mutations to the WAL and replaying the WAL during engine startup. I verified this by inserting `test2`, stopping the server, restarting it, and successfully reading `HELLO_WAL`."
